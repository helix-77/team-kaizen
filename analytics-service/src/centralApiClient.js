  ...
  try {
    ...
    if (err.response?.status === 429) {
      if (attempt >= MAX_RETRIES) {
        throw {
          status: 503,
          body: { error: 'Central API unavailable after 3 retries' },
        };
      }
      ...
      console.log(`[retry ${attempt + 1}/${MAX_RETRIES}] waiting ${Math.round(waitMs / 1000)}s`);
      await new Promise((r) => setTimeout(r, waitMs));
      return centralApiGet(path, params, attempt + 1);
    }
    if (err.response?.status === 404) throw { status: 404, body: err.response.data };
    if (err.response?.status >= 500) throw { status: 502, body: { error: 'Upstream error' } };
    if (err.code === 'ECONNABORTED' || err.code === 'ETIMEDOUT') throw { status: 503, body: { error: 'Unreachable' } };
    throw err;
  }
}
